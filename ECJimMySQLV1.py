...
for thisDC in dcInfo:
    m.addConstr(quicksum(dvars[(thisDCId,thisStoreId)] for (thisDCId,thisStoreId) in dvars.keys() if thisDCId == thisDC[0]), GRB.LESS_EQUAL, thisDC[1])
    
for thisStore in storeInfo:
    m.addConstr(quicksum(dvars[(thisDCId,thisStoreId)] for (thisDCId,thisStoreId) in dvars.keys() if thisStoreId == thisStore[0]), GRB.EQUAL, thisStore[1])
    
"""
for thisDC in dcInfo:
    m.addConstr(quicksum(dvars[tup] for tup in dvars.keys() if tup[0] == thisDC[0]), GRB.LESS_EQUAL, thisDC[1])
    
for thisStore in storeInfo:
    m.addConstr(quicksum(dvars[tup] for tup in dvars.keys() if tup[1] == thisStore[0]), GRB.EQUAL, thisStore[1])
"""
